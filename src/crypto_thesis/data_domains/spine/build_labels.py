...
def spine_build_target_labels(df: pd.DataFrame,
    ...
    final_df = df.merge(std_df, on=["open_time", "target_time"], how="inner")
...
def _build_stdev_by_window(df: pd.DataFrame, df_log_ret: pd.DataFrame) -> pd.DataFrame:
    ...
    for start, end in zip(df["open_time"], df["target_time"]):
        df_filter = df_log_ret[df_log_ret["close_time"].between(start, end)]

        _std = df_filter["log_return"].std(ddof=1) #sampled stdev
        df_aux = pd.DataFrame({"open_time": start, "target_time": end, "std": _std}, index=[0])
